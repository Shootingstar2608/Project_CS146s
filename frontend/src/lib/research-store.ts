...
export type GraphNode = {
  id: string;
  label: string;
  kind: "paper" | "category" | "author" | "year";
};
...
type ResearchState = {
  ...
  updatePaper: (paperId: string, updates: Partial<Pick<Paper, "title" | "category" | "authors" | "year" | "abstract">>) => void;
...
function inferCategory(text: string): PaperCategory {
  ...
  const match = paperCategories.find((category) =>
...
function findRelevantPapers(question: string, papers: Paper[]) {
  ...
  return papers
    .map((paper) => {
      const haystack = `${paper.title} ${paper.category} ${paper.year ?? ""} ${paper.authors.join(" ")} ${paper.abstract ?? ""}`.toLowerCase();
...
function makeAssistantAnswer(question: string, papers: Paper[]) {
  ...
  const sourceLines = sources
    .map((paper, index) => {
      const meta = [paper.authors.join(", "), paper.year, paper.category].filter(Boolean).join(" | ");
...
export function buildKnowledgeGraph(papers: Paper[]) {
  ...
  papers.forEach((paper) => {
    ...
    const categoryId = `category:${paper.category}`;
    nodes.set(categoryId, { id: categoryId, label: paper.category, kind: "category" });
