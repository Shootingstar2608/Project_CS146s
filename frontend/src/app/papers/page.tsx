...
export default function PapersPage() {
  ...
  const filteredPapers = useMemo(() => {
    ...
    return papers.filter((paper) => {
      const searchable = `${paper.title} ${paper.fileName} ${paper.categories?.join(" ") || ""} ${paper.year ?? ""} ${paper.authors?.join(" ")} ${paper.abstract ?? ""}`.toLowerCase();
      const matchesQuery = normalized.length === 0 || searchable.includes(normalized);
      const matchesCategory = category === "all" || paper.categories?.includes(category);
  ...
  const categoryCounts = useMemo(
    () =>
      paperCategories.reduce<Record<PaperCategory, number>>((counts, item) => {
        counts[item] = papers.filter((paper) => paper.categories?.includes(item)).length;
        return counts;
            ...
            <div className={cn(viewMode === "grid" ? "grid gap-5 md:grid-cols-2 xl:grid-cols-3" : "flex flex-col gap-4")}>
              {filteredPapers.map((paper) => (
                <Link key={paper.id} href={`/papers/${paper.id}`}>
                  <article
                    className={cn(
                      "group overflow-hidden rounded-[20px] bg-surface shadow-soft transition-all hover:-translate-y-1 hover:shadow-deep cursor-pointer",
